...
use App\Service\Parser\MainCatalogParser;
...
#[AsCommand(
    name: 'app:parser:main',
    description: 'Runs the main catalog parser and imports products into a category.'
)]
final class MainParserCommand extends Command
{
    public function __construct(
        private readonly MainCatalogParser $parser,
    ...
    {
        $this
            ->addArgument('url', InputArgument::REQUIRED, 'Catalog URL to start parsing from.')
            ->addArgument('category', InputArgument::OPTIONAL, 'Category id or exact category name. For /krovlya/ defaults to "Кровельные материалы".')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $categoryValue = $this->resolveCategoryValue(
            $input->getArgument('category') ? (string) $input->getArgument('category') : null,
            $url
        );

        if ($categoryValue === null) {
            $io->error('Category is required for this URL. Pass category id or exact category name as the second argument.');
        ...
        $io->success(sprintf(
            'Main parser completed. Category: %s. Created: %d. Updated: %d.',
            $category->getName(),
            $created,
            $updated
        ));
    ...
    private function resolveCategoryValue(?string $category, string $url): ?string
    {
        ...
        return match (true) {
            str_contains($path, '/krovlya/') => 'Кровельные материалы',
            default => null,
        };
    }
