...
use App\Service\Parser\InteriorPaintsParser;
...
#[AsCommand(
    name: 'app:parser:interior-paints',
    description: 'Parses LemanaPro interior paints products into the catalog.'
)]
final class InteriorPaintsParserCommand extends Command
{
    public function __construct(
        private readonly InteriorPaintsParser $parser,
    ...
    {
        $this
            ->addArgument('url', InputArgument::OPTIONAL, 'Catalog URL to start parsing from.', InteriorPaintsParser::DEFAULT_URL)
            ->addArgument('category', InputArgument::OPTIONAL, 'Category id or exact category name.', InteriorPaintsParser::CATEGORY_NAME)
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->success(sprintf(
            'Interior paints parser completed. Category: %s. Created: %d. Updated: %d.',
            $category->getName(),
            $created,
            $updated
        ));
