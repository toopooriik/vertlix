...
use App\Service\Parser\ConstructionBlocksParser;
...
#[AsCommand(
    name: 'app:parser:construction-blocks',
    description: 'Parses LemanaPro construction blocks products into the catalog.'
)]
final class ConstructionBlocksParserCommand extends Command
{
    public function __construct(
        private readonly ConstructionBlocksParser $parser,
    ...
    {
        $this
            ->addArgument('url', InputArgument::OPTIONAL, 'Catalog URL to start parsing from.', ConstructionBlocksParser::DEFAULT_URL)
            ->addArgument('category', InputArgument::OPTIONAL, 'Category id or exact category name.', ConstructionBlocksParser::CATEGORY_NAME)
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->success(sprintf(
            'Construction blocks parser completed. Category: %s. Created: %d. Updated: %d.',
            $category->getName(),
            $created,
            $updated
        ));
