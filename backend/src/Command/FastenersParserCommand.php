...
use App\Service\Parser\FastenersParser;
...
#[AsCommand(
    name: 'app:parser:fasteners',
    description: 'Parses LemanaPro fasteners products into the catalog.'
)]
final class FastenersParserCommand extends Command
{
    public function __construct(
        private readonly FastenersParser $parser,
    ...
    {
        $this
            ->addArgument('url', InputArgument::OPTIONAL, 'Catalog URL to start parsing from.', FastenersParser::DEFAULT_URL)
            ->addArgument('category', InputArgument::OPTIONAL, 'Category id or exact category name.', FastenersParser::CATEGORY_NAME)
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->success(sprintf(
            'Fasteners parser completed. Category: %s. Created: %d. Updated: %d.',
            $category->getName(),
            $created,
            $updated
        ));
