...
use App\Service\Parser\LumberParser;
...
#[AsCommand(
    name: 'app:parser:lumber',
    description: 'Parses LemanaPro lumber products into the catalog.'
)]
final class LumberParserCommand extends Command
{
    public function __construct(
        private readonly LumberParser $parser,
    ...
    {
        $this
            ->addArgument('url', InputArgument::OPTIONAL, 'Catalog URL to start parsing from.', LumberParser::DEFAULT_URL)
            ->addArgument('category', InputArgument::OPTIONAL, 'Category id or exact category name.', LumberParser::CATEGORY_NAME)
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->success(sprintf(
            'Lumber parser completed. Category: %s. Created: %d. Updated: %d.',
            $category->getName(),
            $created,
            $updated
        ));
