...
use App\Service\Parser\BathroomFaucetsParser;
...
#[AsCommand(
    name: 'app:parser:bathroom-faucets',
    description: 'Parses LemanaPro bathroom faucets products into the catalog.'
)]
final class BathroomFaucetsParserCommand extends Command
{
    public function __construct(
        private readonly BathroomFaucetsParser $parser,
    ...
    {
        $this
            ->addArgument('url', InputArgument::OPTIONAL, 'Catalog URL to start parsing from.', BathroomFaucetsParser::DEFAULT_URL)
            ->addArgument('category', InputArgument::OPTIONAL, 'Category id or exact category name.', BathroomFaucetsParser::CATEGORY_NAME)
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->success(sprintf(
            'Bathroom faucets parser completed. Category: %s. Created: %d. Updated: %d.',
            $category->getName(),
            $created,
            $updated
        ));
