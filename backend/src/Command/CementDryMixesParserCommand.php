...
use App\Service\Parser\CementDryMixesParser;
...
#[AsCommand(
    name: 'app:parser:cement-dry-mixes',
    description: 'Parses LemanaPro cement and dry mixes products into the catalog.'
)]
final class CementDryMixesParserCommand extends Command
{
    public function __construct(
        private readonly CementDryMixesParser $parser,
    ...
    {
        $this
            ->addArgument('url', InputArgument::OPTIONAL, 'Catalog URL to start parsing from.', CementDryMixesParser::DEFAULT_URL)
            ->addArgument('category', InputArgument::OPTIONAL, 'Category id or exact category name.', CementDryMixesParser::CATEGORY_NAME)
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->success(sprintf(
            'Cement and dry mixes parser completed. Category: %s. Created: %d. Updated: %d.',
            $category->getName(),
            $created,
            $updated
        ));
